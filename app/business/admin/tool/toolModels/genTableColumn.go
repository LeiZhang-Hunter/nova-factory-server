...
import (
	...
	genUtils2 "nova-factory-server/app/business/admin/tool/utils"
...
func GetGenTableColumnDML(column *InformationSchemaColumn, tableId int64, userId int64) *GenTableColumnDML {
	...
	switch {
	case genUtils2.IsTimeObject(dataType): //字段为时间类型
		genTableColumn.GoType = "Time"
		genTableColumn.HtmlType = "datetime"
	case genUtils2.IsNumberObject(dataType): //字段为数字类型
		//字段为数字类型
	...
	default:
		...
		if stringUtils.ReMatchingStr(dataType, "text|tinytext|mediumtext|longtext|longblob") {
			...
			columnLength := genUtils2.GetColumnLength(column.ColumnType)
			if columnLength >= 500 {
				genTableColumn.HtmlType = "textarea"
			}
			genTableColumn.HtmlType = "input"
	...
	//新增字段
	if genUtils2.IsNotEntity(columnName) {
	...
	// 查询字段类型
	if genUtils2.CheckNameColumn(columnName) {
	...
	// 状态字段设置单选框
	if genUtils2.CheckStatusColumn(columnName) {
		genTableColumn.HtmlType = "radio"
	} else if genUtils2.CheckTypeColumn(columnName) || genUtils2.CheckSexColumn(columnName) {
		// 类型&性别字段设置下拉框
