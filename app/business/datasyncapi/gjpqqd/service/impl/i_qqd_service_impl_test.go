// 管家婆全渠道服务实现的单元测试。
// 使用内存缓存（testCache）和内存 SQLite 数据库（shop_goods/shop_goods_sku 表）
// 覆盖 Token 签发/刷新/校验、商品列表查询、库存更新和商品新增等核心场景。
package impl
...
// TestIssueTokenStoresTokensInCacheAndRejectsReusedRefreshToken 测试 Token 签发与刷新完整流程
// 1. 使用授权码换取 token
// 2. 验证 token 有效
// 3. 使用 refresh_token 刷新获取新 token
// 4. 重复使用已消耗的 refresh_token 应失败
...
// TestProductListReadsProductAndSkuTables 测试商品列表查询
// 验证查询结果包含 3 个商品，每个商品的 productid 和 SKU 信息正确
...
// TestProductStockUpdateUpdatesMySQLOnly 测试库存更新
// 1. 不带 skus 参数时按商品批量更新
// 2. 缺少 productid 或缺少 productqty（当无 skus 时）应返回错误
...
// TestAddProductsUpdatesGoodsAndSkuTables 测试商品新增及库存更新
// 1. 直接传入 goodsid 和 quantity 更新库存
// 2. 通过 skus 嵌套数组批量更新多个 SKU 的库存（商品总库存为各 SKU 之和）
...
// testCache 内存缓存实现，用于单元测试中替代 Redis
// 支持 Set/Get/Del/Exists 及 TTL 过期
type testCache struct {
	mu   sync.Mutex
	data map[string]testCacheItem
}

// testCacheItem 缓存项，包含值和过期 deadline
type testCacheItem struct {
	value    string
	deadline time.Time
}

// newTestCache 创建空的内存缓存
...
// Get 从缓存读取，过期或不存在时返回 "cache: nil" 错误
...
// 以下为满足 cache.Cache 接口的空实现方法（测试中未用到）
...
// openTestStockDB 打开内存 SQLite 数据库并创建 shop_goods 和 shop_goods_sku 表
// 预插入 3 个商品和 4 条 SKU 测试数据
...
CREATE TABLE shop_goods_sku (
  ...
  state tinyint DEFAULT 0
	...
	// 预插入测试数据：商品 1001（含 SKU）、2001、2002（含两个 SKU）
...
// assertGoodsQuantity 断言指定商品在数据库中的库存数量
...
// assertSkuQuantity 断言指定 SKU 在数据库中的库存数量
