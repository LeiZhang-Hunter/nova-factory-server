...
func NewShopErpIntegrationConfigDao(db *gorm.DB) dao.ShopErpIIntegrationConfigDao {
	i := &IntegrationConfigDaoImpl{
		db:    db,
		table: "shop_erp_integration_config",
	}
	integration.RegisterStore(i)
	return i
}

func (i *IntegrationConfigDaoImpl) Set(c *gin.Context, req *models.ShopErpIntegrationConfigSet) (*models.ShopErpIntegrationConfig, error) {
	if req.Type == "" {
		return nil, errors.New("type不能为空")
	}

	var data *models.ShopErpIntegrationConfig
	err := i.db.WithContext(c).Transaction(func(tx *gorm.DB) error {
		var exists models.ShopErpIntegrationConfig
		err := tx.Table(i.table).Where("type = ?", req.Type).Where("state = ?", commonStatus.NORMAL).First(&exists).Error
		if errors.Is(err, gorm.ErrRecordNotFound) {
			data = &models.ShopErpIntegrationConfig{
				Type:   req.Type,
				Data:   req.Data,
				Status: req.Status,
...
func (i *IntegrationConfigDaoImpl) List(c *gin.Context, req *models.ShopErpIntegrationConfigQuery) (*models.ShopErpIntegrationConfigListData, error) {
	...
	rows := make([]*models.ShopErpIntegrationConfig, 0)
	if err := db.Order("id DESC").Offset(int((page - 1) * size)).Limit(int(size)).Find(&rows).Error; err != nil {
		return nil, err
	}
	return &models.ShopErpIntegrationConfigListData{
		Rows:  rows,
		Total: total,
	}, nil
}

func (i *IntegrationConfigDaoImpl) GetEnabled(c *gin.Context) (*models.ShopErpIntegrationConfig, error) {
	var item models.ShopErpIntegrationConfig
