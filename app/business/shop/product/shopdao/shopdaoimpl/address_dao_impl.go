...
// NewShopAddressDao 创建商城地址 DAO。
func NewShopAddressDao(ms *gorm.DB) shopdao.IShopAddressDao {
...
// Set 新增或修改商城用户地址，并在设置默认地址时清理同用户的其他默认地址。
func (s *ShopAddressDaoImpl) Set(c *gin.Context, req *shopmodels.AddressSetReq) (*shopmodels.Address, error) {
...
// List 查询商城用户地址列表。
func (s *ShopAddressDaoImpl) List(c *gin.Context, req *shopmodels.AddressQuery) (*shopmodels.AddressListData, error) {
	...
	rows := make([]*shopmodels.Address, 0)
	...
	return &shopmodels.AddressListData{
		Rows:  rows,
		Total: total,
	}, nil
}
...
func (s *ShopAddressDaoImpl) getByIDTx(c *gin.Context, tx *gorm.DB, id int64) (*shopmodels.Address, error) {
	var item shopmodels.Address
