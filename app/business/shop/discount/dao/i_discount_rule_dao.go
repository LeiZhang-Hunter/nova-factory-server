...
type IDiscountRuleDao interface {
	...
	ListByUserAndGoods(c *gin.Context, userID int64, goodsID string, categoryID string) (*models.UserDiscountRule, error)
	// ListValidRulesByTargets 批量查询用户的有效折扣规则（用于列表场景消除 N+1）
	ListValidRulesByTargets(c *gin.Context, userID int64, targetType string, targetIDs []string) ([]*models.UserDiscountRule, error)
}
