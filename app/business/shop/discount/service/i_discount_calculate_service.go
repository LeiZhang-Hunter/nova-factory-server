...
type IDiscountCalculateService interface {
	...
	// originalPrice: 原价
	// 返回: 折扣价, 是否有折扣
	CalculateDiscountPrice(c *gin.Context, userID int64, goodsID string, skuID string, categoryID string, originalPrice float64) (float64, bool)

	// CalculateSkuDiscountPrice 计算SKU的折扣价
	// skus: SKU列表，会直接修改RetailPrice
	// 返回: 修改后的SKU列表, error
	CalculateSkuDiscountPrice(c *gin.Context, userID int64, categoryID string, skus []*SkuPrice) ([]*SkuPrice, error)

	// BatchCalculateDiscountPrices 批量计算商品折扣价（内部批量查库一次，用于列表场景消除 N+1）
	// 返回: map[goodsID]折扣价，仅包含有折扣的商品
	BatchCalculateDiscountPrices(c *gin.Context, userID int64, goods []*GoodsWithPrice) map[string]float64
}

// GoodsWithPrice 商品信息（用于批量折扣计算）
type GoodsWithPrice struct {
	GoodsID     string
	SkuIDs      []string // 该商品的所有 SKU ID 列表
	CategoryID  string
	RetailPrice float64
}
