...
type OrderTimeoutServiceImpl struct {
	...
	db          *gorm.DB
	table       string
	apiService  apiService.IApiShopOrderService
	apiOrderDao dao.IApiShopOrderDao
}

// NewIOrderTimeoutServiceImpl 创建订单超时自动取消服务。
func NewIOrderTimeoutServiceImpl(cache cache.Cache, db *gorm.DB, apiService apiService.IApiShopOrderService, apiOrderDao dao.IApiShopOrderDao) service.IOrderTimeoutService {
...
func (s *OrderTimeoutServiceImpl) cancelOne(id uint64) (bool, error) {
	g := &gin.Context{}
	order, err := s.apiOrderDao.GetByID(g, id)
	...
	err = s.apiService.Cancel(g, int64(order.UserId), int64(id), "超时未支付自动取消")
	if err != nil {
		return false, err
	}
	return true, nil
