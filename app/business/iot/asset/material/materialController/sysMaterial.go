...
func (mi *MaterialInfo) GetMaterialInfoList(c *gin.Context) {
	req := new(materialModels2.MaterialListReq)
...
func (mi *MaterialInfo) SetMaterialInfo(c *gin.Context) {
	info := new(materialModels2.MaterialInfo)
...
func (mi *MaterialInfo) GetMaterialInBoundList(c *gin.Context) {
	req := new(materialModels2.InboundListReq)
	err := c.ShouldBindQuery(req)
	list, err := mi.iMaterialService.InboundList(c, req)
	if err != nil {
		zap.L().Error("读取设备分组失败", zap.Error(err))
		baizeContext.SuccessData(c, &materialModels2.InboundListValue{
			Rows:  make([]*materialModels2.InboundValue, 0),
			Total: 0,
		})
		return
	}
...
func (mi *MaterialInfo) AddMaterialInBound(c *gin.Context) {
	info := new(materialModels2.InboundInfo)
	err := c.ShouldBindJSON(info)
	if err != nil {
		baizeContext.ParameterError(c)
		return
...
func (mi *MaterialInfo) GetMaterialOutBoundList(c *gin.Context) {
	req := new(materialModels2.OutboundListReq)
	err := c.ShouldBindQuery(req)
	list, err := mi.iMaterialService.OutboundList(c, req)
	if err != nil {
		zap.L().Error("读取设备分组失败", zap.Error(err))
		baizeContext.SuccessData(c, &materialModels2.OutboundListValue{
			Rows:  make([]*materialModels2.OutboundValue, 0),
			Total: 0,
		})
		return
	}
...
func (mi *MaterialInfo) AddMaterialOutBound(c *gin.Context) {
	info := new(materialModels2.OutboundInfo)
	err := c.ShouldBindJSON(info)
	if err != nil {
		baizeContext.ParameterError(c)
		return
