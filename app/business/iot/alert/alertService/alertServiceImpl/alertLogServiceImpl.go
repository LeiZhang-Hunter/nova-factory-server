...
type AlertLogServiceImpl struct {
	dao         alertDao2.AlertLogClickhouseDao
	...
	pointDao    deviceDao2.ISysModbusDeviceConfigDataDao
}

func NewAlertLogServiceImpl(dao alertDao2.AlertLogClickhouseDao, ruleDao alertDao2.AlertRuleDao, agentDao daemonizeDao.IotAgentDao, deviceDao deviceDao2.IDeviceDao, buildingDao buildingDao.BuildingDao, pointDao deviceDao2.ISysModbusDeviceConfigDataDao) alertService.AlertLogService {
...
// Export 导出告警数据
func (log *AlertLogServiceImpl) Export(c *gin.Context, data alertModels2.AlertLogData) error {
	// 校验网关帐号密码
	...
	alertLogList, _ := alertModels2.FromDataToNovaAlertLog(&data)
...
func (log *AlertLogServiceImpl) List(c *gin.Context, req *alertModels2.SysAlertLogListReq) (*alertModels2.NovaAlertLogList, error) {
	...
	var rulesMap map[uint64]*alertModels2.SysAlert = make(map[uint64]*alertModels2.SysAlert)
	for _, v := range rules {
...
func (log *AlertLogServiceImpl) Info(c *gin.Context, objectId uint64) (*alertModels2.NovaAlertLog, error) {
