...
type DeviceUtilizationData struct {
	...
	IdlingTime    uint64                   `json:"idling_time"`     //待机时间
	IdlingTimeStr string                   `json:"idling_time_str"` //待机时间
	IdlingRate    float64                  `json:"idling_rate"`     //待机率
	IdlingRateStr string                   `json:"idling_rate_str"` //待机率
	StatusMap     map[int]DeviceStatusData `json:"status_map"`      //各状态稼动率（通用）
}
