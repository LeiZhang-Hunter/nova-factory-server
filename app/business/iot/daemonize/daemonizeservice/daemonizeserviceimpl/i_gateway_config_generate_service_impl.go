...
func (i *iGatewayConfigServiceImpl) Generate(c *gin.Context, gatewayId int64) (*pipeline.PipelineConfig, error) {
	...
	var deviceAddressMap map[string][]*deviceModels2.DeviceVO = make(map[string][]*deviceModels2.DeviceVO)
	var templateIdMap map[uint64]uint64 = make(map[uint64]uint64)
	var templateIds []uint64 = make([]uint64, 0)
	var templatesMap map[uint64][]*deviceModels2.SysModbusDeviceConfigData = make(map[uint64][]*deviceModels2.SysModbusDeviceConfigData)
	...
	for _, device := range devices {
		...
		if device.ProtocolType == device2.MQTT {
			...
			if deviceAddressMap[address] == nil {
				deviceAddressMap[address] = make([]*deviceModels2.DeviceVO, 0)
			}
	...
	err = i.loadPredictionConfig(c, piplines)
	if err != nil {
		zap.L().Error("loadPredictionConfig failed", zap.Error(err))
	}
