...
import (
	...
	daemonizeModels2 "nova-factory-server/app/business/iot/daemonize/daemonizemodels"
...
type DaemonizeServiceImpl struct {
	...
	configDao  daemonizeDao2.IotAgentConfigDao
	manager    *ManagerServiceImpl
	serverList []string
}

func NewDaemonizeServiceImpl(dao daemonizeDao2.IotAgentDao, processDao daemonizeDao2.IotAgentProcess, configDao daemonizeDao2.IotAgentConfigDao) daemonizeservice.DaemonizeService {
...
func (d *DaemonizeServiceImpl) AgentHeartbeat(ctx context.Context, req *v1.AgentHeartbeatReq) (res *v1.AgentHeartbeatRes, err error) {
	...
	agentData = &daemonizeModels2.SysIotAgent{
		ObjectID: uint64(req.GetObjectId()),
		Name:     req.GetName(),
		Version:  req.GetVersion(),
	}
	agentProcessList := make([]*daemonizeModels2.SysIotAgentProcess, 0, len(req.AgentProcessInfo.ProcessList))
	for _, processInfo := range req.AgentProcessInfo.ProcessList {
		agentProcessList = append(agentProcessList, &daemonizeModels2.SysIotAgentProcess{
