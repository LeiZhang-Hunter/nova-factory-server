...
type ControlLogServiceImpl struct {
	...
	deviceDataDao deviceDao2.ISysModbusDeviceConfigDataDao
}

func NewControlLogServiceImpl(dao metricDao.IControlLogDao, deviceDao deviceDao2.IDeviceDao, deviceDataDao deviceDao2.ISysModbusDeviceConfigDataDao) deviceMonitorService.ControlLogService {
...
func (i *ControlLogServiceImpl) List(c *gin.Context, req *deviceMonitorModel.ControlLogListReq) (*metricModels.NovaControlLogList, error) {
	...
	devMap := make(map[uint64]*deviceModels2.DeviceVO, 0)
	...
	devDataMap := make(map[uint64]*deviceModels2.SysModbusDeviceConfigData)
	for _, v := range datas {
