...
type FinancePaymentServiceImpl struct {
	...
	uniqueFields []financePaymentUniqueField
}
...
func NewFinancePaymentService(dao financedao.IFinancePaymentDao) financeservice.IFinancePaymentService {
	return &FinancePaymentServiceImpl{
		dao:          dao,
		uniqueFields: []financePaymentUniqueField{{Field: "No", Column: "no", Label: "付款单号"}},
	}
}
...
func (s *FinancePaymentServiceImpl) create(c *gin.Context, req *financemodels.FinancePaymentUpsert) (*financemodels.FinancePayment, error) {
	...
	financePaymentTrimStringFields(req)
	if err := financePaymentValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *FinancePaymentServiceImpl) update(c *gin.Context, req *financemodels.FinancePaymentUpsert) (*financemodels.FinancePayment, error) {
	...
	id := financePaymentGetIntField(req, "ID")
	if id <= 0 {
...
func (s *FinancePaymentServiceImpl) ListPage(c *gin.Context, req *financemodels.FinancePaymentQuery) (*financemodels.FinancePaymentListData, error) {
	if req != nil {
		financePaymentTrimStringFields(req)
	}
...
func (s *FinancePaymentServiceImpl) validateUniqueFields(c *gin.Context, req *financemodels.FinancePaymentUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := financePaymentGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := financePaymentNormalizeValue(value)
		if empty {
			continue
		}
		...
		if financePaymentGetIntField(exists, "ID") != currentID {
...
type financePaymentUniqueField struct {
	Field  string
	Column string
	Label  string
}

func financePaymentTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	financePaymentTrimStruct(value)
}

func financePaymentValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func financePaymentNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return financePaymentNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func financePaymentGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := financePaymentFieldValue(target, name)
	return value, value.IsValid()
}

func financePaymentGetIntField(target any, name string) int64 {
	value := financePaymentFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func financePaymentTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
		...
		if structField.Anonymous {
			if field.Kind() == reflect.Struct {
				financePaymentTrimStruct(field)
			}
			continue
		}
...
func financePaymentFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
