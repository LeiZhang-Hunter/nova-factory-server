...
type FinancePaymentItemServiceImpl struct {
	...
	uniqueFields []financePaymentItemUniqueField
}
...
func (s *FinancePaymentItemServiceImpl) create(c *gin.Context, req *financemodels.FinancePaymentItemUpsert) (*financemodels.FinancePaymentItem, error) {
	...
	financePaymentItemTrimStringFields(req)
	if err := financePaymentItemValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *FinancePaymentItemServiceImpl) update(c *gin.Context, req *financemodels.FinancePaymentItemUpsert) (*financemodels.FinancePaymentItem, error) {
	...
	id := financePaymentItemGetIntField(req, "ID")
	if id <= 0 {
...
func (s *FinancePaymentItemServiceImpl) ListPage(c *gin.Context, req *financemodels.FinancePaymentItemQuery) (*financemodels.FinancePaymentItemListData, error) {
	if req != nil {
		financePaymentItemTrimStringFields(req)
	}
...
func (s *FinancePaymentItemServiceImpl) validateUniqueFields(c *gin.Context, req *financemodels.FinancePaymentItemUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := financePaymentItemGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := financePaymentItemNormalizeValue(value)
		if empty {
			continue
		}
		...
		if financePaymentItemGetIntField(exists, "ID") != currentID {
...
type financePaymentItemUniqueField struct {
	Field  string
	Column string
	Label  string
}

func financePaymentItemTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	financePaymentItemTrimStruct(value)
}

func financePaymentItemValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func financePaymentItemNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return financePaymentItemNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func financePaymentItemGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := financePaymentItemFieldValue(target, name)
	return value, value.IsValid()
}

func financePaymentItemGetIntField(target any, name string) int64 {
	value := financePaymentItemFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func financePaymentItemTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
		...
		if structField.Anonymous {
			if field.Kind() == reflect.Struct {
				financePaymentItemTrimStruct(field)
			}
			continue
		}
...
func financePaymentItemFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
