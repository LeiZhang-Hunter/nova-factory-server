...
type OrderServiceImpl struct {
	...
	detailDao            saledao.IOrderDetailDao
	accountDao           saledao.IOrderAccountDao
...
func (o *OrderServiceImpl) Sync(event event.OrderEvent) {
	...
	if err := o.syncOrders(salemodels.ToOrder(event)); err != nil {
...
func (o *OrderServiceImpl) syncOrders(orders []*salemodels.Order) error {
	if len(orders) == 0 {
		return nil
	}
	if o.orderDao == nil {
...
func (o *OrderServiceImpl) syncOne(tx *gorm.DB, order *salemodels.Order) error {
	if order == nil {
		return nil
	}
...
func prepareOrderForSave(order *salemodels.Order, now *time.Time) error {
...
func buildERPOrderUpdateMap(order *salemodels.Order, current *salemodels.Order) map[string]any {
