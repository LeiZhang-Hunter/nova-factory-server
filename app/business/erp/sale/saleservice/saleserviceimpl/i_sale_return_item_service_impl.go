...
type SaleReturnItemServiceImpl struct {
	...
	uniqueFields []saleReturnItemUniqueField
}
...
func (s *SaleReturnItemServiceImpl) create(c *gin.Context, req *salemodels.SaleReturnItemUpsert) (*salemodels.SaleReturnItem, error) {
	...
	saleReturnItemTrimStringFields(req)
	if err := saleReturnItemValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *SaleReturnItemServiceImpl) update(c *gin.Context, req *salemodels.SaleReturnItemUpsert) (*salemodels.SaleReturnItem, error) {
	...
	id := saleReturnItemGetIntField(req, "ID")
	if id <= 0 {
...
func (s *SaleReturnItemServiceImpl) ListPage(c *gin.Context, req *salemodels.SaleReturnItemQuery) (*salemodels.SaleReturnItemListData, error) {
	if req != nil {
		saleReturnItemTrimStringFields(req)
	}
...
func (s *SaleReturnItemServiceImpl) validateUniqueFields(c *gin.Context, req *salemodels.SaleReturnItemUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := saleReturnItemGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := saleReturnItemNormalizeValue(value)
		if empty {
			continue
		}
		...
		if saleReturnItemGetIntField(exists, "ID") != currentID {
...
type saleReturnItemUniqueField struct {
	Field  string
	Column string
	Label  string
}

func saleReturnItemTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	saleReturnItemTrimStruct(value)
}

func saleReturnItemValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func saleReturnItemNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return saleReturnItemNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func saleReturnItemGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := saleReturnItemFieldValue(target, name)
	return value, value.IsValid()
}

func saleReturnItemGetIntField(target any, name string) int64 {
	value := saleReturnItemFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func saleReturnItemTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
		...
		if structField.Anonymous {
			if field.Kind() == reflect.Struct {
				saleReturnItemTrimStruct(field)
			}
			continue
		}
...
func saleReturnItemFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
