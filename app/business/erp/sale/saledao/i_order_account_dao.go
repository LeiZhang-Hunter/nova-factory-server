...
// IOrderAccountDao ERP订单账户数据访问接口。
//
// 该接口只提供账户表的单表操作能力，事务边界由 service 层或主 DAO 的 SetWithTx 调用方控制。
type IOrderAccountDao interface {
	// BatchCreate 批量创建保存接口传入的订单账户。
	BatchCreate(tx *gorm.DB, c *gin.Context, orderID uint64, tid string, accounts []*salemodels.OrderAccountSet) error
	...
	ListByOrderIDs(c *gin.Context, orderIDs []uint64) ([]*salemodels.OrderAccount, error)
}
