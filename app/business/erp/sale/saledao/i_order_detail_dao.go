...
type IOrderDetailDao interface {
	...
	BatchCreate(tx *gorm.DB, c *gin.Context, orderID uint64, tid string, details []*salemodels.OrderDetailSet) error
	// BatchCreateByOrder 批量创建事件同步转换后的订单明细。
	BatchCreateByOrder(tx *gorm.DB, orderID uint64, order *salemodels.Order, now *time.Time) error
	...
	DeleteByTidAndOIDs(tx *gorm.DB, tid string, details []*salemodels.OrderDetailSet) error
	// ListByOrderIDs 按订单 ID 集合查询明细记录。
	ListByOrderIDs(c *gin.Context, orderIDs []uint64) ([]*salemodels.OrderDetail, error)
}
