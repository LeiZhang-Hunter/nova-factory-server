...
func (d *PurchaseReturnDaoImpl) Create(c *gin.Context, req *purchasemodels.PurchaseReturnUpsert) (*purchasemodels.PurchaseReturn, error) {
	...
	model := purchasemodels.PurchaseReturnUpsertToEntity(req)
	if model == nil {
		return nil, errors.New("参数不能为空")
	}
...
func (d *PurchaseReturnDaoImpl) Update(c *gin.Context, req *purchasemodels.PurchaseReturnUpsert) (*purchasemodels.PurchaseReturn, error) {
	...
	db := d.db.WithContext(c).Table("erp_purchase_return").Where("id = ?", req.ID)
...
func (d *PurchaseReturnDaoImpl) DeleteByIDs(c *gin.Context, ids []int64) error {
	...
	return d.db.WithContext(c).Table("erp_purchase_return").
		Where("id IN ?", ids).
...
func (d *PurchaseReturnDaoImpl) GetByID(c *gin.Context, id int64) (*purchasemodels.PurchaseReturn, error) {
	...
	if err := d.db.WithContext(c).Table("erp_purchase_return").
		Where("id = ?", id).
		Where("state = ?", commonStatus.NORMAL).
...
func (d *PurchaseReturnDaoImpl) ListPage(c *gin.Context, req *purchasemodels.PurchaseReturnQuery) (*purchasemodels.PurchaseReturnListData, error) {
	if req == nil {
		req = new(purchasemodels.PurchaseReturnQuery)
	}
	db := d.db.WithContext(c).Table("erp_purchase_return").Where("state = ?", commonStatus.NORMAL)
	db = applyPurchaseReturnFilters(db, req)
	...
	return &purchasemodels.PurchaseReturnListData{Rows: result, Total: total}, nil
}
