...
type PurchaseReturnServiceImpl struct {
	...
	uniqueFields []purchaseReturnUniqueField
}
...
func NewPurchaseReturnService(dao purchasedao.IPurchaseReturnDao) purchaseservice.IPurchaseReturnService {
	return &PurchaseReturnServiceImpl{
		dao:          dao,
		uniqueFields: []purchaseReturnUniqueField{{Field: "No", Column: "no", Label: "采购退货单号"}},
	}
}
...
func (s *PurchaseReturnServiceImpl) create(c *gin.Context, req *purchasemodels.PurchaseReturnUpsert) (*purchasemodels.PurchaseReturn, error) {
	...
	purchaseReturnTrimStringFields(req)
	if err := purchaseReturnValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *PurchaseReturnServiceImpl) update(c *gin.Context, req *purchasemodels.PurchaseReturnUpsert) (*purchasemodels.PurchaseReturn, error) {
	...
	id := purchaseReturnGetIntField(req, "ID")
	if id <= 0 {
...
func (s *PurchaseReturnServiceImpl) ListPage(c *gin.Context, req *purchasemodels.PurchaseReturnQuery) (*purchasemodels.PurchaseReturnListData, error) {
	if req != nil {
		purchaseReturnTrimStringFields(req)
	}
...
func (s *PurchaseReturnServiceImpl) validateUniqueFields(c *gin.Context, req *purchasemodels.PurchaseReturnUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := purchaseReturnGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := purchaseReturnNormalizeValue(value)
		if empty {
			continue
		}
		...
		if purchaseReturnGetIntField(exists, "ID") != currentID {
...
type purchaseReturnUniqueField struct {
	Field  string
	Column string
	Label  string
}

func purchaseReturnTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	purchaseReturnTrimStruct(value)
}

func purchaseReturnValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func purchaseReturnNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return purchaseReturnNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func purchaseReturnGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := purchaseReturnFieldValue(target, name)
	return value, value.IsValid()
}

func purchaseReturnGetIntField(target any, name string) int64 {
	value := purchaseReturnFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func purchaseReturnTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
		...
		if structField.Anonymous {
			if field.Kind() == reflect.Struct {
				purchaseReturnTrimStruct(field)
			}
			continue
		}
...
func purchaseReturnFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
