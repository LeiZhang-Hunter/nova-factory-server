package purchasecontroller

import (
	"nova-factory-server/app/business/erp/purchase/purchasemodels"
	"nova-factory-server/app/business/erp/purchase/purchaseservice"
	"nova-factory-server/app/middlewares"
	"nova-factory-server/app/utils/baizeContext"

	"github.com/gin-gonic/gin"
)

// PurchaseIn ERP 采购入库控制器
type PurchaseIn struct {
	service purchaseservice.IPurchaseInService
}

// NewPurchaseIn 创建 ERP 采购入库控制器。
func NewPurchaseIn(service purchaseservice.IPurchaseInService) *PurchaseIn {
	return &PurchaseIn{service: service}
}

// PrivateRoutes 注册 ERP 采购入库私有路由。
func (o *PurchaseIn) PrivateRoutes(router *gin.RouterGroup) {
	group := router.Group("/erp/purchase/in")
	group.GET("/list", middlewares.HasPermission("erp:purchase:in:list"), o.List)
	group.GET("/query/:id", middlewares.HasPermission("erp:purchase:in:query"), o.GetByID)
	group.POST("/set", middlewares.HasPermission("erp:purchase:in:set"), o.Set)
	group.DELETE("/remove/:ids", middlewares.HasPermission("erp:purchase:in:remove"), o.Delete)
}

// List 查询 ERP 采购入库列表。
// @Summary 查询 ERP 采购入库列表
// @Description 按条件分页查询 ERP 采购入库列表
// @Tags ERP/采购管理
// @Security BearerAuth
// @Param object query purchasemodels.PurchaseInQuery true "ERP 采购入库查询参数"
// @Produce application/json
// @Success 200 {object} response.ResponseData "查询成功"
// @Router /erp/purchase/in/list [get]
func (o *PurchaseIn) List(c *gin.Context) {
	req := new(purchasemodels.PurchaseInQuery)
	if err := c.ShouldBindQuery(req); err != nil {
		baizeContext.ParameterError(c)
		return
	}
	data, err := o.service.List(c, req)
	if err != nil {
		baizeContext.Waring(c, err.Error())
		return
	}
	baizeContext.SuccessData(c, data)
}

// GetByID 查询 ERP 采购入库详情。
// @Summary 查询 ERP 采购入库详情
// @Description 根据ID查询 ERP 采购入库详情
// @Tags ERP/采购管理
// @Security BearerAuth
// @Param id path int true "主键ID"
// @Produce application/json
// @Success 200 {object} response.ResponseData "查询成功"
// @Router /erp/purchase/in/query/{id} [get]
func (o *PurchaseIn) GetByID(c *gin.Context) {
	id := baizeContext.ParamInt64(c, "id")
	if id == 0 {
		baizeContext.ParameterError(c)
		return
	}
	data, err := o.service.GetByID(c, id)
	if err != nil {
		baizeContext.Waring(c, err.Error())
		return
	}
	baizeContext.SuccessData(c, data)
}

// Set 新增或修改 ERP 采购入库。
// @Summary 新增或修改 ERP 采购入库
// @Description 新增或修改 ERP 采购入库
// @Tags ERP/采购管理
// @Security BearerAuth
// @Accept application/json
// @Param body body purchasemodels.PurchaseInUpsert true "ERP 采购入库参数"
// @Produce application/json
// @Success 200 {object} response.ResponseData "设置成功"
// @Router /erp/purchase/in/set [post]
func (o *PurchaseIn) Set(c *gin.Context) {
	req := new(purchasemodels.PurchaseInUpsert)
	if err := c.ShouldBindJSON(req); err != nil {
		baizeContext.ParameterError(c)
		return
	}
	var (
		data *purchasemodels.PurchaseIn
		err  error
	)
	if req.ID > 0 {
		data, err = o.service.Update(c, req)
	} else {
		data, err = o.service.Create(c, req)
	}
	if err != nil {
		baizeContext.Waring(c, err.Error())
		return
	}
	baizeContext.SuccessData(c, data)
}

// Delete 删除 ERP 采购入库。
// @Summary 删除 ERP 采购入库
// @Description 根据ID删除 ERP 采购入库，多个ID用逗号分隔
// @Tags ERP/采购管理
// @Security BearerAuth
// @Param ids path string true "主键ID，多个用逗号分隔"
// @Produce application/json
// @Success 200 {object} response.ResponseData "删除成功"
// @Router /erp/purchase/in/remove/{ids} [delete]
func (o *PurchaseIn) Delete(c *gin.Context) {
	ids := baizeContext.ParamInt64Array(c, "ids")
	if len(ids) == 0 {
		baizeContext.ParameterError(c)
		return
	}
	if err := o.service.DeleteByIDs(c, ids); err != nil {
		baizeContext.Waring(c, err.Error())
		return
	}
	baizeContext.Success(c)
}
