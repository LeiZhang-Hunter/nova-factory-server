...
func (d *StockOutItemDaoImpl) Create(c *gin.Context, req *stockmodels.StockOutItemUpsert) (*stockmodels.StockOutItem, error) {
	...
	model := stockmodels.StockOutItemUpsertToEntity(req)
	if model == nil {
		return nil, errors.New("参数不能为空")
	}
...
func (d *StockOutItemDaoImpl) Update(c *gin.Context, req *stockmodels.StockOutItemUpsert) (*stockmodels.StockOutItem, error) {
	...
	db := d.db.WithContext(c).Table("erp_stock_out_item").Where("id = ?", req.ID)
...
func (d *StockOutItemDaoImpl) DeleteByIDs(c *gin.Context, ids []int64) error {
	...
	return d.db.WithContext(c).Table("erp_stock_out_item").
		Where("id IN ?", ids).
...
func (d *StockOutItemDaoImpl) GetByID(c *gin.Context, id int64) (*stockmodels.StockOutItem, error) {
	...
	if err := d.db.WithContext(c).Table("erp_stock_out_item").
		Where("id = ?", id).
		Where("state = ?", commonStatus.NORMAL).
...
func (d *StockOutItemDaoImpl) ListPage(c *gin.Context, req *stockmodels.StockOutItemQuery) (*stockmodels.StockOutItemListData, error) {
	if req == nil {
		req = new(stockmodels.StockOutItemQuery)
	}
	db := d.db.WithContext(c).Table("erp_stock_out_item").Where("state = ?", commonStatus.NORMAL)
	db = applyStockOutItemFilters(db, req)
	...
	return &stockmodels.StockOutItemListData{Rows: result, Total: total}, nil
}
