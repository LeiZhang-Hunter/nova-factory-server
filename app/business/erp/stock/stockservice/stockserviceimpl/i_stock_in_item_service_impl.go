...
type StockInItemServiceImpl struct {
	...
	uniqueFields []stockInItemUniqueField
}
...
func (s *StockInItemServiceImpl) create(c *gin.Context, req *stockmodels.StockInItemUpsert) (*stockmodels.StockInItem, error) {
	...
	stockInItemTrimStringFields(req)
	if err := stockInItemValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *StockInItemServiceImpl) update(c *gin.Context, req *stockmodels.StockInItemUpsert) (*stockmodels.StockInItem, error) {
	...
	id := stockInItemGetIntField(req, "ID")
	if id <= 0 {
...
func (s *StockInItemServiceImpl) ListPage(c *gin.Context, req *stockmodels.StockInItemQuery) (*stockmodels.StockInItemListData, error) {
	if req != nil {
		stockInItemTrimStringFields(req)
	}
...
func (s *StockInItemServiceImpl) validateUniqueFields(c *gin.Context, req *stockmodels.StockInItemUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := stockInItemGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := stockInItemNormalizeValue(value)
		if empty {
			continue
		}
		...
		if stockInItemGetIntField(exists, "ID") != currentID {
...
type stockInItemUniqueField struct {
	Field  string
	Column string
	Label  string
}

func stockInItemTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	stockInItemTrimStruct(value)
}

func stockInItemValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func stockInItemNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return stockInItemNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func stockInItemGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := stockInItemFieldValue(target, name)
	return value, value.IsValid()
}

func stockInItemGetIntField(target any, name string) int64 {
	value := stockInItemFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func stockInItemTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
...
func stockInItemFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
