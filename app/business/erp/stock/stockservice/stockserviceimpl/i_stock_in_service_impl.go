...
type StockInServiceImpl struct {
	...
	uniqueFields []stockInUniqueField
}
...
func NewStockInService(dao stockdao.IStockInDao) stockservice.IStockInService {
	return &StockInServiceImpl{
		dao:          dao,
		uniqueFields: []stockInUniqueField{{Field: "No", Column: "no", Label: "入库单号"}},
	}
}
...
func (s *StockInServiceImpl) create(c *gin.Context, req *stockmodels.StockInUpsert) (*stockmodels.StockIn, error) {
	...
	stockInTrimStringFields(req)
	if err := stockInValidateRequiredFields(req); err != nil {
		return nil, err
	}
...
func (s *StockInServiceImpl) ListPage(c *gin.Context, req *stockmodels.StockInQuery) (*stockmodels.StockInListData, error) {
	if req != nil {
		stockInTrimStringFields(req)
	}
...
func (s *StockInServiceImpl) validateUniqueFields(c *gin.Context, req *stockmodels.StockInUpsert, currentID int64) error {
	...
	for _, field := range s.uniqueFields {
		value, ok := stockInGetFieldValue(req, field.Field)
		if !ok {
			continue
		}
		normalized, empty := stockInNormalizeValue(value)
		if empty {
			continue
		}
		...
		if stockInGetIntField(exists, "ID") != currentID {
...
type stockInUniqueField struct {
	Field  string
	Column string
	Label  string
}

func stockInTrimStringFields(target any) {
	if target == nil {
		return
	}
	...
	stockInTrimStruct(value)
}

func stockInValidateRequiredFields(target any) error {
	if target == nil {
		return nil
	}
	value := reflect.ValueOf(target)
...
func stockInNormalizeValue(value reflect.Value) (any, bool) {
	if !value.IsValid() {
		return nil, true
	}
	if value.Kind() == reflect.Pointer {
		...
		return stockInNormalizeValue(value.Elem())
	}
	switch value.Kind() {
	case reflect.String:
...
func stockInGetFieldValue(target any, name string) (reflect.Value, bool) {
	value := stockInFieldValue(target, name)
	return value, value.IsValid()
}

func stockInGetIntField(target any, name string) int64 {
	value := stockInFieldValue(target, name)
	if !value.IsValid() {
		return 0
	}
...
func stockInTrimStruct(value reflect.Value) {
	for i := 0; i < value.NumField(); i++ {
		field := value.Field(i)
...
func stockInFieldValue(target any, name string) reflect.Value {
	value := reflect.ValueOf(target)
	if value.Kind() == reflect.Pointer {
		if value.IsNil() {
