...
// CollectorService 采集器服务实现
type CollectorService struct {
	collectorDao dao.ICollectorDAO
}

// NewCollectorService 创建服务
func NewCollectorService(collectorDao dao.ICollectorDAO) service.ICollectorService {
	return &CollectorService{collectorDao: collectorDao}
}

// CreateCollector 创建采集器
func (s *CollectorService) CreateCollector(c *gin.Context, req *dto.CreateCollectorRequest) (*entity.Collector, error) {
	// 设备 ID 唯一性检查
	if req.DeviceID != "" {
...
// GetCollector 获取采集器
func (s *CollectorService) GetCollector(c *gin.Context, id string) (*entity.Collector, error) {
...
// ListCollectors 列表
func (s *CollectorService) ListCollectors(c *gin.Context, offset, limit int, name, deviceID, status string) ([]entity.Collector, int64, error) {
	return s.collectorDao.List(c, offset, limit, name, deviceID, status)
}

// ListAllOnline 获取全部在线采集器（用于下发弹窗）
func (s *CollectorService) ListAllOnline(c *gin.Context) ([]entity.Collector, error) {
...
func (s *CollectorService) UpdateCollector(c *gin.Context, id string, req *dto.UpdateCollectorRequest) (*entity.Collector, error) {
	col, err := s.collectorDao.GetByID(c, id)
	if err != nil {
		return nil, err
	}
...
// DeleteCollector 删除
func (s *CollectorService) DeleteCollector(c *gin.Context, id string) error {
	col, err := s.collectorDao.GetByID(c, id)
	if err != nil {
		return err
	}
