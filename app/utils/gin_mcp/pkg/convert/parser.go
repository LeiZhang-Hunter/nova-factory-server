...
func processRouterOperation(parser *Parser, operation *Operation) error {
	...
	for _, routeProperties := range operation.RouterProperties {
		...
		operationID := strings.ToUpper(routeProperties.HTTPMethod) + strings.ReplaceAll(strings.ReplaceAll(routeProperties.Path, "/", "_"), ":", "")

		// Check for duplicates even with default IDs (shouldn't happen but be defensive)
		if usedOperationIDs[operationID] {
			log.Printf("Warning: Duplicate operation ID '%s' for route %s %s. Skipping this route.", operationID,
				routeProperties.HTTPMethod, routeProperties.Path)
			continue
		}
		...
		if isDebugMode() {
			log.Printf("Processing route: %s %s -> OpID: %s", routeProperties.HTTPMethod, routeProperties.Path, operationID)
		}

		// Generate description information
		description := fmt.Sprintf("Handler for %s %s", routeProperties.HTTPMethod, routeProperties.Path)
		...
		parser.NewTools = append(parser.NewTools, tool)
		parser.Operations[operationID] = types.Operation{
			Method: routeProperties.HTTPMethod,
