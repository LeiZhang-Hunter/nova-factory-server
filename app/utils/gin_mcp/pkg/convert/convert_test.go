...
func TestParseHandlerCommentsByIdentity_UsesGeneratedDocsWithoutSource(t *testing.T) {
	identity := handlerIdentity{
		PackagePath: "nova-factory-server/app/utils/gin_mcp/pkg/convert",
		HandlerName: "customOpHandler",
	}

	doc, err := parseHandlerCommentsByIdentity(identity, "<autogenerated>", "customOpHandler")
	require.NoError(t, err)
	require.NotNil(t, doc)
	assert.Equal(t, "Custom operation handler", doc.Summary)
	assert.Equal(t, "myCustomOp", doc.OperationID)
}
...
// MethodHandler handles route metadata fallback tests.
func (testRouteHandler) MethodHandler(c *gin.Context) {
...
func Test_getRouteHandlerInfo_MethodValueFallback(t *testing.T) {
	gin.SetMode(gin.ReleaseMode)
	r := gin.New()
	controller := testRouteHandler{}
	r.GET("/method", controller.MethodHandler)

	routes := r.Routes()
	require.Len(t, routes, 1)

	filePath, funcName := getRouteHandlerInfo(routes[0])

	assert.Equal(t, "MethodHandler", funcName)
	assert.NotEmpty(t, filePath)
	assert.NotEqual(t, "<autogenerated>", filePath)
	assert.True(t, strings.HasSuffix(filePath, ".go"))
}

func Test_parseHandlerIdentity_MethodValue(t *testing.T) {
	identity := parseHandlerIdentity("nova-factory-server/app/utils/gin_mcp/pkg/convert.(*testRouteHandler).MethodHandler-fm")

	assert.Equal(t, "nova-factory-server/app/utils/gin_mcp/pkg/convert", identity.PackagePath)
	assert.Equal(t, "testRouteHandler", identity.ReceiverName)
	assert.Equal(t, "MethodHandler", identity.HandlerName)
}
