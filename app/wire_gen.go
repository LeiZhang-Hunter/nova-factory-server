...
//go:generate go run -mod=mod github.com/google/wire/cmd/wire gen -tags "ai iot shop erp"
//go:build !wireinject
// +build !wireinject
...
func wireApp() (*gin.Engine, func(), error) {
	...
	iShopOrderDao := impl5.NewShopOrderDao()
	iShopOrderItemDao := impl5.NewShopOrderItemDao()
	iShopOrderService := impl6.NewShopOrderService(iShopOrderDao, iShopOrderItemDao, iShopUserDao)
	order := shopcontroller2.NewOrder(iShopOrderService)
	...
	iApiShopCartDao := impl7.NewIApiShopCartDaoImpl(db)
	iApiShopCartService := impl8.NewShopCartServiceImpl(iShopGoodsDao, iShopSkuDao, iShopUserDao, iApiShopCartDao)
	productCart := product.NewCart(iApiShopCartService)
