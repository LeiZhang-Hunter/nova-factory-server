...
//go:generate go run -mod=mod github.com/google/wire/cmd/wire gen -tags "iot erp shop ai"
//go:build !wireinject
// +build !wireinject
...
import (
	...
	impl7 "nova-factory-server/app/business/shop/api/dao/impl"
	impl8 "nova-factory-server/app/business/shop/api/service/impl"
	shopcontroller3 "nova-factory-server/app/business/shop/config/controller"
	impl9 "nova-factory-server/app/business/shop/config/dao/impl"
	impl10 "nova-factory-server/app/business/shop/config/service/impl"
...
func wireApp() (*gin.Engine, func(), error) {
	...
	iGatewayConfigService := daemonizeserviceimpl.NewIGatewayConfigServiceImpl(iDeviceDao, iDeviceTemplateDao, iSysModbusDeviceConfigDataDao, iCameraDao, alertRuleDao, alertSinkTemplateDao, iDeviceElectricDao, iDictDataDao, iAiPredictionControlDao)
	...
	agent := gatewaycontroller.NewAgent(iaiAgentService, iAiModelProviderService, iaiAgentDao, imcpServerDao, iAiLLMDao, iDictDataDao)
	...
	shopcontrollerAddress := shopcontroller2.NewAddress(iShopAddressService)
	...
	controller6 := &shopcontroller2.Controller{
		Address: shopcontrollerAddress,
		Cart:    cart,
		User:    shopcontrollerUser,
	}
	iShopWechatUserDao := impl7.NewShopWechatUserDao(db)
	iAppShopAuthService := impl8.NewShopAuthService(cacheCache, iShopWechatUserDao)
	iShopSysConfigDao := impl7.NewShopSysConfigDao(db)
	iAppShopWechatAuthService := impl8.NewShopWechatAuthService(cacheCache, iShopSysConfigDao, iShopWechatUserDao)
	authAuth := auth.NewAuth(iAppShopAuthService, iAppShopWechatAuthService)
	...
	iShopOrderDao := impl7.NewIShopOrderDaoImpl(db)
	iShopOrderItemDao := impl7.NewIShopOrderItemDaoImpl(db)
	iAppShopOrderService := impl8.NewIShopOrderServiceImpl(iShopOrderDao, iShopOrderItemDao, iShopWechatUserDao)
	orderOrder := order.NewOrder(iAppShopOrderService)
	daoIShopAddressDao := impl7.NewShopAddressDao(db)
	iAppShopAddressService := impl8.NewShopAddressService(daoIShopAddressDao)
	addressAddress := address.NewAddress(iAppShopAddressService)
	daoIShopSysConfigDao := impl9.NewShopSysConfigDao(db)
	iShopSysConfigService := impl10.NewShopSysConfigService(daoIShopSysConfigDao)
	wechatConfig := shopcontroller3.NewWechatConfig(iShopSysConfigService)
	controller7 := &shopcontroller3.Controller{
		WechatConfig: wechatConfig,
	}
	shopShop := shop.NewGinEngine(app, cacheCache, controllerController, controller5, shopcontrollerController, controller6, authController, productController, orderOrder, addressAddress, controller7)
	...
	iLogisticsCompanyDao := settingdaoimpl.NewLogisticsCompanyDao(db)
	iLogisticsCompanyService := settingserviceimpl.NewLogisticsCompanyService(iLogisticsCompanyDao)
	logistics := settingcontroller.NewLogistics(iLogisticsCompanyService)
	...
	ordercontrollerOrder := ordercontroller.NewOrder(iOrderService)
